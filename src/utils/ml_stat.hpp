...
public:
  ...
  std::queue<std::pair<std::pair<uint64_t, uint64_t>, uint32_t>>
  op_time_log_q;
  std::queue<std::pair<std::pair<uint64_t, uint64_t>, uint32_t>>
  op_time_log_q_compute;
  std::queue<std::pair<std::pair<uint64_t, uint64_t>, uint32_t>>
  op_time_log_q_network;
};
...
public:
  ...
  void set_epoch_parameters(uint32_t epoch_num,
			    double* model,
			    const sst::MLSST* ml_sst);
  void collect_results(uint32_t epoch_num,
		       ml_model::ml_model* ml_model,
		       std::string ml_model_name);
  void log_num_lost_gradients(int node_id,
			      int sgd_type,
			      sst::MLSST* ml_sst);
